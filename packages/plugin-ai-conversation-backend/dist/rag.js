...
exports.buildContextMessage = buildContextMessage;
/**
...
async function retrieveContext(opts) {
    ...
    const perStore = await Promise.all(vectorStoreIds.map(async (id) => {
        ...
        if (!upstream.ok) {
            ...
            throw new Error(`vector store ${id} search ${upstream.status}: ${text || upstream.statusText}`);
        }
...
function buildContextMessage(results) {
    ...
    return {
        id: 'kb-context',
        role: 'system',
        content: `Answer the user's question using the knowledge base excerpts below.` +
